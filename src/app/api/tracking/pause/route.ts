// POST /api/tracking/pause — mettre le suivi EN PAUSE (arrêt/repos) — chauffeur uniquement.
...
import { pauseTracking } from "@/services/tracking/tracking.service";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await pauseTracking(auth);
    return ok(result);
  } catch (err) {
    return routeError(err, "POST /api/tracking/pause");
  }
}
