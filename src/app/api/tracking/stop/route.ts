// POST /api/tracking/stop — TERMINER LE TRAJET (chauffeur) — chauffeur uniquement.
...
import { stopTracking } from "@/services/tracking/tracking.service";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await stopTracking(auth);
    return ok(result);
  } catch (err) {
    return routeError(err, "POST /api/tracking/stop");
  }
}
