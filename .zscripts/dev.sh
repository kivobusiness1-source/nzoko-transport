...
# Anti-reboot (Task 30) : si la base vient d'être recréée par db:push
# (reboot sandbox → tar → .env réécrit), elle est VIERGE → seed
# automatique NON FATAL. Une base déjà peuplée n'est jamais re-seedée.
log_step_start "Auto-seed si base vierge"
echo "[BUN] Vérification base vierge…"
if bun run scripts/db-is-empty.ts; then
        echo "[BUN] Base VIERGE — seed initial (comptes, villes, agences, voyages)…"
        bun prisma/seed.ts || echo "⚠️  Seed principal échoué (non fatal — vérifier db/custom.db)"
        bun run scripts/seed-v3.ts || echo "⚠️  Seed V3 échoué (non fatal)"
else
        echo "[BUN] Base déjà peuplée — pas de seed."
fi
log_step_end "Auto-seed si base vierge"
